    ...
    <el-card v-else-if="survey" class="survey-card">
      ...
      <div v-for="(q, index) in survey.questions" :key="q.id" class="question-item">
        ...
        <h3>
          ...
          <el-tag size="small" effect="plain" style="margin-left: 10px">{{ getLabel(q.type) }}</el-tag>
        </h3>
        
        <el-radio-group v-if="q.type === 'SINGLE'" v-model="answers[q.id]">
          <el-radio v-for="opt in q.config.options" :key="opt" :label="opt" size="large" border class="block-radio">{{ opt }}</el-radio>
        </el-radio-group>

        <el-checkbox-group v-if="q.type === 'MULTI'" v-model="answers[q.id]">
          <el-checkbox v-for="opt in q.config.options" :key="opt" :label="opt" size="large" border class="block-radio">{{ opt }}</el-checkbox>
        </el-checkbox-group>

        <el-select v-if="q.type === 'DROPDOWN'" v-model="answers[q.id]" placeholder="请选择" size="large" style="width: 100%">
          <el-option v-for="opt in q.config.options" :key="opt" :label="opt" :value="opt" />
        </el-select>

        <el-rate v-if="q.type === 'RATING'" v-model="answers[q.id]" :max="q.config.max || 5" allow-half show-score />
        ...
        <div v-if="['FILE','IMAGE','VIDEO','AUDIO'].includes(q.type)">
           <el-upload
             :action="'/api/upload/' + uploadTypeMap(q.type)"
           ...
           <div v-if="answers[q.id]" class="preview-box">
              ...
              已上传成功
              <img v-if="q.type === 'IMAGE'" :src="'http://localhost:8080' + answers[q.id]" class="preview-img" />
              <audio v-if="q.type === 'AUDIO'" controls :src="'http://localhost:8080' + answers[q.id]" class="preview-audio"></audio>
           </div>
        </div>
        ...
        <div v-if="q.type === 'SIGN'">
           ...
           <div style="margin-top: 10px;">
             <el-button type="primary" size="small" @click="handleGenerateSign(q.id)">确认使用签名</el-button>
      ...
      <div class="footer">
        <el-button type="primary" size="large" @click="submit" :loading="submitting" style="width: 200px;">
          提交问卷
        </el-button>
      </div>
...
const getLabel = (type) => {
  const map = {
    ...
    'RATING': '评分题', 'TEXT': '文本题', 'DATE': '日期',
    'RANK': '排序题', 'FILE': '文件', 'IMAGE': '图片',
    'VIDEO': '视频', 'AUDIO': '音频', 'SIGN': '签名'
  }
  return map[type] || '题目'
}
...
const handleGenerateSign = (qid) => {
  ...
  esignInstance.generate().then(res => {
...
<style scoped>
.main-container { max-width: 800px; margin: 20px auto; padding: 0 10px; }
.loading-state { text-align: center; padding: 50px; color: #909399; }
.survey-title { text-align: center; color: #303133; margin-bottom: 10px; }
.survey-desc { text-align: center; color: #606266; margin-bottom: 30px; }
.question-item { margin-bottom: 30px; padding: 20px; border-radius: 8px; background: #fff; border: 1px solid #ebeef5; box-shadow: 0 2px 12px 0 rgba(0,0,0,0.05); }
.question-item h3 { margin-top: 0; margin-bottom: 15px; font-size: 16px; color: #303133; }
.block-radio { width: 100%; margin-left: 0 !important; margin-bottom: 10px; }
.upload-demo { margin-bottom: 10px; }
.preview-box { margin-top: 10px; padding: 10px; background: #f0f9eb; border-radius: 4px; color: #67C23A; }
.preview-img { max-width: 200px; display: block; margin-top: 5px; border-radius: 4px; }
.preview-audio { width: 100%; margin-top: 5px; }
.sign-board { border: 1px solid #dcdfe6; border-radius: 4px; overflow: hidden; }
